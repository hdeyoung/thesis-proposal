...
Using ordered logic, we can specify the behavior of an incrementable binary counter.
...
Operationally, increments are described by three clauses, which together constitute the specification's \vocab{signature}, $\sig_{\inc}$.
...
similarly, by rewriting $\bit{0} \fuse \inc$ as $\bit{1}$, the third clause ensures that in the $\bit{0}$ case the carry flips the $\bit{0}$ to $\bit{1}$.
...
More generally, the above clauses adequately specify the increment operation on binary numbers.
% , as discussed further in \cref{sec:gener-invar}.
...
\begin{equation*}
  \begin{tikzcd}[row sep=1ex, arrow style=math font]
    ...
    \eps \fuse \bit{1} \fuse \mathul{\bit{0} \fuse \inc}
      \drar[start anchor=base east, end anchor={[yshift=1ex]base west}]
    \\
    \mathul{\eps \fuse \inc} \fuse \mathul{\bit{0} \fuse \inc}
      \urar[start anchor={[yshift=1ex]base east}, end anchor=base west]
      \drar[start anchor=base east, end anchor={[yshift=1ex]base west}]
    &&
...
\footnote{In the above example, indistinguishability of the two different interleavings is tantamount to commutativity of the diagram.% requiring the diagram to commute.
}
Then, because we can't observe which rewriting occurred first, the two rewritings appear to happen concurrently.
This is the idea of \vocab{concurrent equality}~\autocites{Watkins+:CMU02}{Cervesato+:LFMTP12} from the \acs{CLF} framework for linear logical specifications, later adapted by \textcite{Simmons:CMU12} for ordered logical specifications.
...
As a further example, it's also possible to extend the binary counter specification with support for decrements.
...
As it passes over any $\bit{0}$s at the least significant end, they are marked as $\bit[']{0}$ atoms to indicate that they are waiting to borrow from their more significant neighbors:
\begin{equation*}
  \bit{0} \fuse \dec \lrimp \monad{\dec \fuse \bit[']{0}} \,.
\end{equation*}
Whenever the $\dec$ reaches the $\eps$ or right-most $\bit{1}$, it is replaced with either $\fail$ or $\suc$, respectively, to show whether the borrow was possible; in the case of $\bit{1}$, the borrow is also effected:
\begin{equation*}
  \begin{lgathered}
  ...
  \end{lgathered}
\end{equation*}
Then the $\fail$ or $\suc$ travels back over all of the $\bit[']{0}$ atoms that were waiting to borrow.
...
\begin{equation*}
  \begin{tikzcd}[row sep=1ex, arrow style=math font]
    ...
    \mathul{\eps \fuse \inc} \fuse \mathul{\bit{0} \fuse \dec}
      ...
      \drar[start anchor=base east, end anchor={[yshift=1ex]base west}]
    &&
    \eps \fuse \mathul{\bit{1} \fuse \dec} \fuse \bit[']{0}
      \trans \dotsb
    \\
    &
    \mathul{\eps \fuse \inc} \fuse \dec \fuse \bit[']{0}
      \urar[start anchor={[yshift=1ex]base east}, end anchor=base west]
  \end{tikzcd}
\end{equation*}
% \begin{align*}
...
\Cref{fig:binary-counter-summary} summarizes the increment and decrement programs.
...
\subsection{Infinite traces% and fairness
}\label{sec:olp-intuition:infinite-traces}
...
Transition unfairness is fundamentally at odds with concurrency because it admits the possibility that one event precludes another, independent event---just as the persistent rewriting of $\incs$ precludes the rewriting of the independent $\eps \fuse \inc$ here.
Therefore, we will implicitly assume that all traces are weakly transition-fair.

% Transition unfairness is fundamentally at odds with concurrency.
% Because concurrency allows multiple independent events to occur simultaneously, one event should not preclude another independent event;
% in the above example, for instance, rewriting the $\incs$ atom should not preclude rewriting the independent $\eps \fuse \inc$ substring.
...
% \textcolor{red}{Move this section to proposed work.}

% Thus far, we have used ordered logic to specify concurrent systems, whereas the non-modal fragment of ordered logic was originally developed by \textcite{Lambek:AMM58} to describe sentence structure.
