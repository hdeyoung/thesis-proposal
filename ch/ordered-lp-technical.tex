...
In this \lcnamecref{sec:ordered-lp:technical}, we review the technical details, confining ourselves to a propositional fragment but otherwise generally following the lead of \citeauthor{Simmons:CMU12}'s SLS framework~\autocite*{Simmons:CMU12}.
...
\begin{figure}[!tbp]
  \begin{infers}[Inversion: $\octx \seq \nof{N : A^-}$][ with $\octx$ stable]
    \infer[\rlab{\rimp}]{\octx \seq \nof{\rlam{p.N} : A^+ \rimp B^-}}{
      \octx_{A^+} \pseq \pof{p : A^+} &
      \octx, \octx_{A^+} \seq \nof{N : B^-}}
    \and
    \infer[\rlab{\limp}]{\octx \seq \nof{\llam{p.N} : A^+ \limp B^-}}{
      \octx_{A^+} \pseq \pof{p : A^+} &
      \octx_{A^+}, \octx \seq \nof{N : B^-}}
    \and
    \infer[\rlab{\with}]{\octx \seq \nof{\pair{N_1 , N_2} : A^-_1 \with A^-_2}}{
      \octx \seq \nof{N_1 : A^-_1} &
      \octx \seq \nof{N_2 : A^-_2}}
    \and
    \infer[\rlab{\monad{}}]{\octx \seq \nof{\lett{T in V} : \monad{A^+}}}{
      \tof{T :: \octx \trans* \octx'} &
      \octx' \seq \vof{V : [A^+]}}
  \end{infers}
...
\begin{figure}
  \begin{infers}[Pattern: $\octx \pseq \pof{p : A^+}$]
...
\begin{figure}
  \begin{infers}[$\tof{T :: \octx \trans* \octx'}$]
    ...
    \and
    \infer{\tof{\tstep{p <- R; T} :: \omatch{\octx} \trans* \octx'}}{
      \octx \seq \aof{R : \susp-{\monad{A^+}}} &
      \octx_{A^+} \pseq \pof{p : A^+} &
      \tof{T :: \ofill{\octx_{A^+}} \trans* \octx'}}
  \end{infers}
...
\begin{figure}
  \begin{infers}[Stable: $\octx \seq \aof{R : \susp-{C^-}}$]
    \infer{\omatch{x{:}A^-} \seq \aof{\atm{x . S} : \susp-{C^-}}}{
      \ofill{\lfoc{A^-}} \seq \sof{S : \susp-{C^-}}}
    \and
...
\begin{figure}[!t]
  \begin{infers}[Left-focus: $\omatch{\lfoc{A^-}} \seq \sof{S : \susp-{C^-}}$]
    \infer{\omatch{\lfoc{A^-}} \seq \sof{\snil : \susp-{A^-}}}{
      }
    \and
    \infer{\omatch{\lfoc{A^+ \rimp B^-}, \octx} \seq \sof{\rapp{V ; S} : \susp-{C^-}}}{
      \octx \seq \vof{V : \rfoc{A^+}} &
      \ofill{\lfoc{B^-}} \seq \sof{S : \susp-{C^-}}}
    \and
    \infer{\omatch{\octx, \lfoc{A^+ \limp B^-}} \seq \sof{\lapp{V ; S} : \susp-{C^-}}}{
      \octx \seq \vof{V : \rfoc{A^+}} &
      \ofill{\lfoc{B^-}} \seq \sof{S : \susp-{C^-}}}
    \and
    \infer{\omatch{\lfoc{A^-_1 \with A^-_2}} \seq \sof{\fst{S} : \susp-{C^-}}}{
      \ofill{\lfoc{A^-_1}} \seq \sof{S : \susp-{C^-}}}
    \and
    \infer{\omatch{\lfoc{A^-_1 \with A^-_2}} \seq \sof{\snd{S} : \susp-{C^-}}}{
      \ofill{\lfoc{A^-_2}} \seq \sof{S : \susp-{C^-}}}    
  \end{infers}
  \caption{Spines}
\end{figure}

\begin{figure}
  \begin{infers}[Right-focus: $\octx \seq \vof{V : \rfoc{A^+}}$]
    \infer{x{:}\susp+{\p^+} \seq \vof{x : \rfoc{\p^+}}}{
      }
    \and
