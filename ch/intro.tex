...
Proof-construction-as-computation views the search for a proof, according to a fixed strategy, as the basis [source] [origin] of computation, and is the foundation for logic programming~\autocites{Miller+:PAL91}{Andreoli:JLC92};
...
Although these same concurrent systems can be simulated according to their \ac{CLF} specifications by the Lollimon~\autocite{Lopez+:PPDP05} and Celf~\autocite{Schack-Nielsen:ITU11} logic programming engines, the programs ultimately remain specifications, not distributed implementations.
...
Moreover, the proof-reduction approach yields actual distributed implementations~\autocite{Toninho+:ESOP13}.

At first glance, the proof-construction and proof-reduction approaches to concurrent computation appear strikingly disparate in spite of their common [logical] basis in linear logic.
The two have different dynamics, provide different guarantees, and serve different roles in software engineering practice.
