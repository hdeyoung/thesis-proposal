...
\begin{equation*}
  ...
  \,.
\end{equation*}
From a string rewriting perspective, this implication is a\fxnote{\ \st{rewriting}} rule for rewriting the (sub)string $\bit{1} \fuse \inc$ as $\inc \fuse \bit{0}$%
...
For example, under the $\sig_{\inc}$ signature, the counter $\eps \fuse \bit{1} \fuse \inc$ can be maximally rewritten as in the trace
...
\begin{equation*}
  ...
  \text{\,,}
\end{equation*}
where at each step the sites available for rewriting have been underlined.%
...
We will frequently omit the index when it is clear from the context.}
...
This is the idea of \vocab{concurrent equality} from {CLF}~\autocite{Watkins+:CMU02}, borrowed for ordered logic programming, which justifies the adoption of true concurrency as an abstraction.
...
As a forward-chaining ordered logic program, this game is specified by the signature, $\sig_{\coin}$, consisting of the four clauses:
\begin{align*}
