...
Using ordered logic, we can specify an incrementable binary counter.
...
More generally, the clauses adequately specify an increment operation, as discussed further in \cref{sec:gener-invar}.
...
\footnote{In the above example, indistinguishability of the two different interleavings is tantamount to requiring the diagram to commute.}
Then, because we can't observe which rewriting occurred first, the two rewritings appear to happen concurrently.
This is the idea of \vocab{concurrent equality} from the {CLF}~\autocite{Watkins+:CMU02} linear logical specification framework, adapted by \textcite{Simmons:CMU12} for ordered logical specifications.
...
Transition unfairness is fundamentally at odds with concurrency because it allows the possibility that one event precludes another, independent event, as does the persistant rewriting of $\incs$ preclude the rewriting of the independent $\eps \fuse \inc$.
