...
In this section, we present a reformulation of \citeauthor{Caires+:MSCS13}'s \ac{SILL} for a restriction of intuitionistic linear logic, which we call singleton linear logic.
As hinted below and further justified in \cref{?}, \ac{SISLL} is a better fit for comparisons with concurrent ordered logical specifications.
...
\begin{gather*}
  ...
  % \scaleleftright[0.4em]{\lbag}{
    \infer[{\lrule{\with}}]{\oseq{\hypof{\with*{\ell: A_\ell}[\ell \in L]} |- \procof{\selectL{\kay; Q}}{C}}}{
      \oseq{\hypof{A_{\kay}} |- \procof{Q}{C}} &
      \text{($\kay \in L$)}}
  % }{\rbag}_{k \in I}
  % }}[k \in I]$}
  \\[2\jot]
  ...
  % \scaleleftright[0.4em]{\lbag}{
    \begin{lgathered}
      ...
      \exec{(\caseR*{\ell => P_\ell}[\ell \in L])} \fuse \msgL{\kay} \lrimp \monad{\exec{P_{\kay}}} \mathrlap{\qquad\text{($\kay \in L$)}}
    \end{lgathered}
  % }{\rbag}_{k \in I}
  % \,}[k \in I]
\end{gather*}
...
\begin{equation*}
  ...
  {\oseq{\pctx ; \lctx |- \procof{\mletrec[X \in \mathcal{X}]{(\mprocdef{X{:}\ctxmonad{A_X <- \lctx_X} = P_X})}{Q}}{C}}}{
...
\begin{equation*}
  \begin{lgathered}
    \exec{(\mletrec[X \in \mathcal{X}]{(\mprocdef{X = P_X})}{Q})} \lrimp \monad{\bagged{\bang \bnd{X = P_X}}[X \in \mathcal{X}] \fuse \exec{Q}}
    \\
    \bang \bnd{X = P} \fuse \exec{X} \lrimp \monad{\exec{P}}
  \end{lgathered}
\end{equation*}
The environment of bindings of process varibles to process expressions is represented in the \ac{SSOS} as a collection of $\bnd{X = P}$ hypotheses.
...
\begin{gather*}
  % \bagged{
    \infer[{\rrule{\ssor}}]{\oseq{\lctx |- \procof{\selectR{\kay; P}}{\ssor*{\ell: A_\ell}[\ell \in L]}}}{
      \oseq{\lctx |- \procof{P}{A_{\kay}}} &
      \text{($\kay \in L$)}}
%   }[k \in I]
  \qquad
  ...
  % \bagged{
    \begin{lgathered}
      ...
      \msgR{\kay} \fuse \exec{(\caseL*{\ell => Q_\ell}[\ell \in L])} \lrimp \monad{\exec{Q_{\kay}}} \mathrlap{\qquad\text{($\kay \in L$)}}
    \end{lgathered}
  % }[k \in I]
\end{gather*}
...
\begin{mathpar}
  \infer[\rrule{\one}]{\oseq{\lctxe |- \procof{\closeR}{\one}}}{
    }
  \and
  \infer[\lrule{\one}]{\oseq{\one |- \procof{\waitL{Q}}{C}}}{
    \oseq{\lctxe |- \procof{Q}{C}}}
\end{mathpar}
The operational semantics is asynchronous, with the $\closeR$ process sending a $\msgQ$ message:
\begin{equation*}
  \begin{lgathered}
...
For multiplicative conjunction, the left rule is problematic because it breaks one hypothesis into two; for linear implication, the right rule is problematic because it introduces a new hypothesis even if one is already there.
