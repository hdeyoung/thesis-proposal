...
Thus far, we have followed a proof-construction approach to computation, having in \cref{?} reviewed an interpretation of ordered logical specifications as concurrent string rewriting and in \cref{?} identified a fragment in which those specifications have a message-passing character.
In this \lcnamecref{sec:sill}, we turn to a proof-reduction view of computation.

Recently, \textcite{Caires+Pfenning:CONCUR10} with Toninho~\autocite*{Caires+:MSCS13} have established a Curry--Howard isomorphism, dubbed \acs{SILL}, between the sequent calculus for intuitionistic linear logic and a session-typed $\pi$-calculus, in which propositions are session types, proofs are session-typed processes, and cut reductions are process reductions.
This gives a proof-reduction view of concurrency that differs from the proof-construction perspective.

In this section, we present a reformulation of \citeauthor{Caires+:MSCS13}'s \ac{SILL} that

\subsection{Why singleton linear logic?}
...
Because a process offers its service along a distinguished channel, the basic session-typing judgment of \citeauthor{Caires+:MSCS13}'s \ac{SILL} is $P :: x{:}A$, meaning \enquote{process $P$ offers service of type $A$ along channel $x$}.
...
Just as the inference rules arrange sequents in a proof tree, so do the \ac{SILL} session-typing rules arrange processes in a tree-shaped network in which some processes are clients of more than one process (i.e., some nodes have more than one child).
...
\begin{equation*}
  \begin{tikzpicture}[channel/.style = {text depth=0, midway, sloped}]
    \graph [
...
In this thesis proposal, we are interested in a restriction of \ac{SILL} that will match ordered logical specifications.
To match the ordered context of these specifications, we cannot directly use \ac{SILL}---at least not in its full generality.
We instead want a restriction of \ac{SILL} in which process networks are chains, not arbitrary trees.


One might expect that process chains should arise from ordered logic.
But, taking into account how the structure of the \ac{SILL} session-typing judgment induces a tree-shape for process networks, it becomes apparant that process chains arise when processes are restricted to use at most one service each.
That is, when contexts $\lctx$ are restricted to be either empty or singletons, the session-typing judgment becomes
\begin{equation*}
  \lctxe \vdash P :: x{:}A
  \quad\text{or}\quad
  x_1{:}A_1 \vdash P :: x{:}A
...
Its worth emphasizing that we still have a Curry--Howard isomorphism in the presence of the restriction to singleton antecedents: although we choose to present the logical rules and process assignment simultaneously, singleton linear logic is a well-defined logic
...
\begin{equation*}
  \infer[\cut{A}]{\oseq{\lctx |- \procof{\spawn{P; Q}}{C}}}{
    \oseq{\lctx |- \procof{P}{A}} &
    \oseq{A |- \procof{Q}{C}}}
  \,,
\end{equation*}
...
\begin{equation*}
  ...
  \end{lgathered}
\end{equation*}
To execute the selection process $\selectL{\inl ; Q}$, we asynchronously send to the left a message containing the label $\inl$, which is represented in the \ac{SSOS} as the proposition $\msgL{\inl}$, and then immediately continue the session by executing $Q$, \ie\ process $Q$.
...
\begin{gather*}
  \infer[\rrule{\with}]{\oseq{\lctx |- \procof{\caseR*{\ell => P_\ell}[\ell \in L]}{\with*{\ell: A_\ell}[\ell \in L]}}}{
    \forall \ell \in L \mathpunct{:}\enskip \oseq{\lctx |- \procof{P_\ell}{A_\ell}}}
  \qquad
  % \raisebox{0.5\baselineskip}{$\bagged{\,\raisebox{-0.5\baselineskip}{
  % \scaleleftright[0.4em]{\lbag}{
    \infer[{\lrule{\with}}]{\oseq{\hypof{\with*{\ell: A_\ell}[\ell \in L]} |- \procof{\selectL{\ell'; Q}}{C}}}{
      \oseq{\hypof{A_{\ell'}} |- \procof{Q}{C}} &
      \text{($\ell' \in L$)}}
  % }{\rbag}_{k \in I}
  % }}[k \in I]$}
  \\[2\jot]
  ...
  % \scaleleftright[0.4em]{\lbag}{
    \begin{lgathered}
      \exec{(\selectL{\ell' ; Q})} \lrimp \monad{\msgL{\ell'} \fuse \exec{Q}} \\
      \exec{(\caseR*{\ell => P_\ell}[\ell \in L])} \fuse \msgL{\ell'} \lrimp \monad{\exec{P_{\ell'}}} \mathrlap{\qquad\text{($\ell' \in L$)}}
    \end{lgathered}
  % }{\rbag}_{k \in I}
  % \,}[k \in I]
\end{gather*}
...
% turn out to
introduce a communication overhead because we would be sending multiple $\inj{\mathsf{1/2}}$s separately rather than a single label $\ell'$.
...
However, to keep matters simpler, we instead choose to rely on general recursion and must content ourselves with the departure from a pure Curry--Howard isomorphism.
...
\begin{equation*}
  ...
  {\oseq{\pctx ; \lctx |- \procof{\mletrec{\bagged{\mprocdef{X{:}\ctxmonad{A_X <- \lctx_X} = P_X}}[X \in \mathcal{X}]}{Q}}{C}}}{
    \text{($\pctx' = \bagged{X{:}\ctxmonad{A_X <- \lctx_X}}[X \in \mathcal{X}]$)}
    &
    % \bagged{
    \forall X \in \mathcal{X}\mathpunct{:}\enskip
      \oseq{\pctx, \pctx' ; \lctx_X |- \procof{P_X}{A_X}}
    % }[j \in I]
    &
    \oseq{\pctx, \pctx' ; \lctx |- \procof{Q}{C}}}
  \,.
\end{equation*}

\begin{equation*}
  \begin{lgathered}
    \exec{(\mletrec{\bagged{\mprocdef{X = P_X}}[X \in \mathcal{X}]}{Q})} \lrimp \monad{\bagged{\bang \bnd{X = P_X}}[X \in \mathcal{X}] \fuse \exec{Q}}
    \\
...
This service, \sillinline`Counter`, is therefore a recursive (unary) additive conjunction, declared in the concrete syntax as \sillinline`stype Counter = &{ inc: Counter }`.
The \sillinline`eps` process offers this service outright, and thus has type \sillinline`{|- Counter}`.
The \sillinline`bit0` and \sillinline`bit1` processes, on the other hand, use the service offered by their more significant neighbors, and thus have type \sillinline`{Counter |- Counter}`.
...
Note, however, that this adequacy property is not enforced by the type \sillinline`Counter`.
An appropriate dependent session type could enforce increment adequacy, but, for simplicity of exposition, we prefer the simple type for now.
...
\begin{mathpar}
  \infer[{\rrule[1]{\ssor}}]{\oseq{\lctx |- A_1 \ssor A_2}}{
    \oseq{\lctx |- A_1}}
  \and
  \infer[{\rrule[2]{\ssor}}]{\oseq{\lctx |- A_1 \ssor A_2}}{
    \oseq{\lctx |- A_2}}
  \and
  \infer[\lrule{\ssor}]{\oseq{A_1 \ssor A_2 |- C}}{
    \oseq{A_1 |- C} &
    \oseq{A_2 |- C}}
\end{mathpar}
...
\begin{mathpar}
  \infer[{\rrule[1]{\ssor}}]{\oseq{\lctx |- \procof{\selectR{\inl; P}}{A_1 \ssor A_2}}}{
    \oseq{\lctx |- \procof{P}{A_1}}}
  \and
  \infer[{\rrule[2]{\ssor}}]{\oseq{\lctx |- \procof{\selectR{\inr; P}}{A_1 \ssor A_2}}}{
    \oseq{\lctx |- \procof{P}{A_2}}}
  \and
  \infer[\lrule{\ssor}]{\oseq{A_1 \ssor A_2 |- \procof{\caseL{\inl => Q_1 | \inr => Q_2}}{C}}}{
    \oseq{A_1 |- \procof{Q_1}{C}} &
    \oseq{A_2 |- \procof{Q_2}{C}}}
\end{mathpar}
...
\begin{equation*}
  ...
  \end{lgathered}
\end{equation*}
It's important to distinguish the $\msgR{}$ predicate, which represents messages that flow to the right, from the $\msgL{}$ predicate so that a selection process's continuation does not mistakenly capture the message that was just sent.
This could otherwise happen in executing $\selectR{\inl ; \caseR{\inl => P_1 | \inr => P_2}}$, for example.
...
\begin{gather*}
  % \bagged{
    \infer[{\rrule{\ssor}}]{\oseq{\lctx |- \procof{\selectR{\ell'; P}}{\ssor*{\ell: A_\ell}[\ell \in L]}}}{
      \oseq{\lctx |- \procof{P}{A_{\ell'}}} &
      \text{($\ell' \in L$)}}
%   }[k \in I]
  \qquad
  \infer[\lrule{\ssor}]{\oseq{\ssor*{\ell: A_\ell}[\ell \in L] |- \procof{\caseL*{\ell => Q_\ell}[\ell \in L]}{C}}}{
    \forall \ell \in L\mathpunct{:}\enskip \oseq{A_\ell |- \procof{Q_\ell}{C}}}
  \\[2\jot]
  % \bagged{
    \begin{lgathered}
      \exec{(\selectR{\ell'; P})} \lrimp \monad{\exec{P} \fuse \msgR{\ell'}} \\
      \msgR{\ell'} \fuse \exec{(\caseL*{\ell => Q_\ell}[\ell \in L])} \lrimp \monad{\exec{Q_{\ell'}}} \mathrlap{\qquad\text{($\ell' \in L$)}}
    \end{lgathered}
  % }[k \in I]
\end{gather*}
...
\begin{sillcode}[
  ...
  stype Counter = &{ inc: Counter , dec: Counter' }
    and Counter' = +{ ok: Counter, fail: Counter }
...
According to the type declaration, a process offering the \sillinline`Counter` service gives its client a choice of increment or decrement services.
If the client chooses to decrement, the offering process will choose to reply with either \sillinline`fail` or \sillinline`ok` and then recursively offer the \sillinline`Counter` service.
...
Once again, these adequacy properties are not enforced by the type \sillinline`Counter`, although they could be with an appropriate dependent session type.


\subsection{Identity as forwarding}\label{sec:ident-as-forw}

In singleton linear logic, there is also an identity principle that states that one way to obtain a resource is to directly use an existing resource.
\begin{equation*}
  \infer[\id{A}]{\oseq{A |- A}}{
    }
\end{equation*}
...
\begin{equation*}
  \infer[\id{A}]{\oseq{A |- \procof{\fwd}{A}}}{
    }
  \,.
\end{equation*}
Rather than making the forwarding explicit in the operational semantics, we can simply eliminate the middleman, linking the neighboring processes:
\begin{equation*}
