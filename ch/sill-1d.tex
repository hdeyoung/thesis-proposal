...
Because a process offers its service along a distinguished channel, \ac{SILL}'s basic session-typing judgment is $P :: x{:}A$, meaning \enquote{process $P$ offers service of type $A$ along channel $x$}.
...
they are the rules from the linear sequent calculus, but restricted to contexts with at most one hypothesis.  

When restricted to singleton or empty antecedents, the linear sequent calculus rules for the additive conjunction $A_1 \with A_2$ are as follows.
