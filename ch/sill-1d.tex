...
Thus far, we have followed a proof-construction approach to computation, having in \cref{?} reviewed an interpretation of ordered logical specifications as concurrent string rewriting and in \cref{?} identified a fragment in which specifications have a message-passing character.
...
The linear sequent calculus's inference rules thus become session-typing rules for \ac{SILL} processes.
Just as the inference rules arrange sequents in a proof tree, so do the session-typing rules arrange \ac{SILL} processes in a tree-shaped network, in which some processes are clients of more than one process (i.e., some nodes have more than one child).
...
To match the ordered context of these specifications, we cannot directly use \ac{SILL}---we instead want a restriction of \ac{SILL} in which process networks are chains, not general trees.
...
\begin{equation*}
  ...
  \,.
\end{equation*}
Thus, to execute the $\spawn{}$, we execute the processes $P$ and $Q$ side-by-side, with process $P$ offering a service that [[the] continuation process] $Q$ uses.
...
To make the language more palatable for the programmer, we diverge slightly from a pure propositions-as-types interpretation of singleton linear logic by including $n$-ary labeled additive conjunctions as a primitive.
...
By analogy with the binary conjunction, the typing rules and operational semantics for $\with*{\ell_i: A_i}[i \in I]$ are as follows.
\begin{gather*}
...
However, to keep matters simpler, we instead choose to rely on general recursion, contenting ourselves with the departure from a pure Curry--Howard isomorphism.
...
\begin{equation*}
  \infer[
    \text{\scshape letrec}
    \quad
    {\scriptstyle (\pctx_I = \bagged{X_i{:}\ctxmonad{A_i <- \lctx_i}}[i \in I])}
  ]
  {\oseq{\pctx ; \lctx |- \procof{\mletrec{\bagged{\mprocdef{X_i{:}\ctxmonad{A_i <- \lctx_i} = P_i}}[i \in I]}{Q}}{C}}}{
    \bagged{
      \oseq{\pctx, \pctx_I ; \lctx_j |- \procof{P_j}{A_j}}
    }[j \in I]
    &
    \oseq{\pctx, \pctx_I ; \lctx |- \procof{Q}{C}}}
  \,.
\end{equation*}
%
...
This service, \sillinline`Counter`, is therefore a recursive additive conjunction, declared in the concrete syntax as \sillinline`stype Counter = &{ inc: Counter }`.
...
\begin{equation*}
  \begin{lgathered}
    \exec{(\selectR{\inl ; P})} \lrimp \monad{\exec{P} \fuse \msg{\inl}} \\
    \msg{\inl} \fuse \exec{(\caseL{\inl => Q_1 | \inr => Q_2})} \lrimp \monad{\exec{Q_1}}
    %
    \\[\jot]
    %
    \exec{(\selectR{\inr ; P})} \lrimp \monad{\exec{P} \fuse \msg{\inr}} \\
    \msg{\inr} \fuse \exec{(\caseL{\inl => Q_1 | \inr => Q_2})} \lrimp \monad{\exec{Q_2}}
      \,.
  \end{lgathered}
\end{equation*}

Once again, to make the language more convenient for the programmer, we include $n$-ary labeled additive disjunctions $\ssor*{\ell_i: A_i}[i \in I]$.
The typing rules and operational semantics are thus more generally
\begin{gather*}
  \bagged{
    \infer[{\rrule[k]{\ssor}}]{\oseq{\lctx |- \procof{\selectR{\ell_k; P}}{\ssor*{\ell_i: A_i}[i \in I]}}}{
      \bagged{\oseq{\lctx |- \procof{P}{A_i}}}[i \in I]}
  }[k \in I]
  \\
  \infer[\lrule{\ssor}]{\oseq{\ssor*{\ell_i: A_i}[i \in I] |- \procof{\caseL*{\ell_i => Q_i}[i \in I]}{C}}}{
    \bagged{\oseq{A_i |- \procof{Q_i}{C}}}[i \in I]}
  \\[2\jot]
  \bagged{
    \begin{lgathered}
      \exec{(\selectR{\ell_k; P})} \lrimp \monad{\exec{P} \fuse \msg{\ell_k}} \\
      \msg{\ell_k} \fuse \exec{(\caseL*{\ell_i => Q_i}[i \in I])} \lrimp \monad{\exec{Q_k}}
    \end{lgathered}
  }[k \in I]
\end{gather*}
