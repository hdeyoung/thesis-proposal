...
For example, the counter $\eps \fuse \bit{1} \fuse \inc$ can be rewritten as
% % \newcolumntype{b}[1]{>{\centering\arraybackslash\hspace{0pt}$}p{#1}<{$}}%
% \newcommand*{\threebits}[4][0.85em]{%
%   $\mathsf{#2\,#3\,#4}$%
% %   \begin{tabular}{@{}b{#1}@{}b{#1}@{}b{#1}@{}}
% %     \mathsf{#2}&\mathsf{#3}&\mathsf{#4}
% %   \end{tabular}
% }%
% \newcommand*{\fourbits}[5][0.85em]{%
%   $\mathsf{#2\,#3\,#4\,#5}$%
%   % \begin{tabular}{@{}b{#1}@{}b{#1}@{}b{#1}@{}b{#1}@{}}
%   %   \mathsf{#2}&\mathsf{#3}&\mathsf{#4}&\mathsf{#5}
%   % \end{tabular}
% }%
\ExplSyntaxOn
...
%   \begin{tikzpicture}
%   [
%     baseline, every node/.append style = {anchor = base},
%     start chain, node distance = 1.5em, 
%     every node/.append style = {on chain, join},
%     every join/.append style = {->}
%   ]
%     \node {$\eps \fuse \bit{1} \fuse \inc$};
%     \node {$\eps \fuse \inc \fuse \bit{0}$};
%     \node {$\eps \fuse \bit{1} \fuse \bit{0}$};
%     % \node {\threebits{e}{b_1}{i}};
%     % \node {\threebits{e}{i}{b_0}};
%     % \node {\threebits{e}{b_1}{b_0}};
%   \end{tikzpicture}%
%   ,
% \end{center}
% at which point no more rewrites are possible.
where the rewrite sites available at each step are underlined;
this trace computes $1 + 1 = 2$ in binary representation.\fxnote{\ \st{%
More generally, the above rewrite rules adequately implement an increment operation:
...
The above ordered logic program for increments has a progress property:
...
%     every join/.append style = {->}
%   ]
%     \node {$\eps \fuse \bit{1} \fuse \bit{0} \fuse \dec$};
%     \node {$\eps \fuse \bit{1} \fuse \dec \fuse \bit[']{0}$};
%     \node {$\eps \fuse \bit{0} \fuse \succ \fuse \bit[']{0}$};
%     \node {$\eps \fuse \bit{0} \fuse \bit{1} \fuse \succ$};
%   \end{tikzpicture}%
%   ,
% \end{center}
at which point no more rewrites are possible.
...
\NewDocumentCommand{\susp}{m}{\langle#1\rangle}
...
\NewDocumentCommand{\atm}{m m}{#1 \cdot #2}
\NewDocumentCommand{\rlam}{m m}{\lambda #1.#2}
\NewDocumentCommand{\llam}{m m}{\lambda #1.#2}
