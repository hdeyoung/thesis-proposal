...
% % Processes are not omniscient but instead execute locally, with asynchronous processes communicating through messages that can be sent and received \enquote{without ever leaving home}.
% % processes do not communicate on an arbitrary scale but instead communicate 
% How are these views related?
% % It's natural to wonder how these views are related:
...
In contrast, concurrency is traditionally phrased in formal calculi as the composition of \vocab{communicating processes}:
...
Inspired by the process-as-formula view of linear logic~\autocites{Miller:ELP92}{Cervesato+Scedrov:IC09}, we propose that each atomic proposition in an ordered logical specification % serves in
be assigned one of two roles:
an atom is either process-like or message-like.
These role assignments, or \vocab{choreographies}, refine the original specification: 
...
Ideally, choreographies would be mechanically generated from % ordered logical 
specifications.
...
% And some specifications even admit several choreographies, among which the programmer should exercise his choice.
